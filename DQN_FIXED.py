...
class FlockingEnv(gym.Env):
    ...
    def read_agent_locations(self):
        ...
        File = os.path.join(*(Results['InitPositions']+str(self.counter)).split('\\'), "config.json")
        # print(File)
...
model = DQN(policy="MlpPolicy", env=env, verbose=1, tensorboard_log="./ppo_Agents_tensorboard/")
eval_interval = 500
eval_callback = EvalCallback(env, eval_freq=eval_interval, deterministic=True, render=False)

model.set_random_seed(random_seed)
model.learn(total_timesteps=10000, callback=eval_callback)
print("Done")
# model.save(rf"{Files['Flocking']}\\Models\\FlockingNew")
model.save(os.path.join(Files['Flocking'], "Models", "FlockingNew"))
...
positions_directory = os.path.join(Files['Flocking'], "Testing","Positions")
velocities_directory = os.path.join(Files['Flocking'], "Testing", "Dynamics", "Velocities")
accelerations_directory = os.path.join(Files['Flocking'], "Testing", "Dynamics", "Accelerations")
