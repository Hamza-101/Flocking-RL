...
class FlockingEnv(gym.Env):
    def __init__(self, agents):
        ...
        self.action_space = spaces.Box(low=-1.0, high=5.0, shape=(2,), dtype=np.float32)  # Control over velocity in x and y axes
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(4,), dtype=np.float32)  # Position and velocity in x and y axes
        self.CollisionThreshold = 3
    ...
    def step(self, actions):
        ...
        for i, action in enumerate(actions):
                agent = self.flocking_agents[i]
                agent.acceleration = np.array(action, dtype=float)
                agent.velocity = np.clip(agent.velocity + agent.acceleration, -agent.max_velocity, agent.max_velocity)
                agent.position += agent.velocity

        done = False  
        for agent in self.flocking_agentsagents:
            reward[agent]=self.calculate_reward(agent)

        total_reward = sum(reward)
        obs = np.array([[agent.position[0], agent.position[1], agent.velocity[0], agent.velocity[1]] for agent in self.flocking_agents], dtype=np.float32)
        return obs, total_reward, done

    def reset(self):
        agent_locations = read_agent_locations()
        self.flocking_agents=[Agent(position) for position in agent_locations]
        return 
    ...
    def calculate_reward(self):
        for agent in self.flocking_agents:
            ...
            # Use cohesion_reward, separation_reward, and collision_penalty as needed
...
for episode in range(1, episodes + 1):
    ...
    while not done:
        ...
        obs, reward, done= env.step(action) # Not using info
        reward_episode += reward
    print('Episode:{} Score:{} '.format(episode, reward_episode))
