...
class FlockingEnv(gym.Env):
    ...
    def calculate_reward(self):
        ...
        for _, agent in enumerate(self.agents): 
            ...
            alignment, out_of_flock=self.reward1(agent, neighbor_velocities)   
            cohesion_separation = self.reward2(agent, neighbor_positions)        
            # collision_reward += self.CollisionRewardCal(agent, neighbor_positions)

            reward =  alignment + cohesion_separation
    ...
    # CTDE
    def reward1(self, agent, neighbor_velocities):
        out_of_flock=False
        total_reward=0
        multiplier=(len(neighbor_velocities))
        ...
        if (len(neighbor_velocities) > 0):
                ...
                total_reward =  10*multiplier * (1 - np.abs(orientation_diff))

        elif (len(neighbor_velocities) == 0):
            out_of_flock=True
            total_reward -= 100
    ...
    def reward2(self, agent, neighbor_positions):
        ...
        if (len(neighbor_positions) > 0):
            for neighbor_position in neighbor_positions:
                ...
                if (distance < SimulationVariables["SafetyRadius"]) :
                    total_reward -= 100 * (1 - (distance / SimulationVariables["SafetyRadius"]))

                # Check this won't hold if too far away and CTDE                              
                elif (SimulationVariables["SafetyRadius"] < distance < SimulationVariables["NeighborhoodRadius"]):
                    total_reward += 100

        #CENTER OF MASS            
        elif(len(neighbor_positions) == 0):
            total_reward = -100 
