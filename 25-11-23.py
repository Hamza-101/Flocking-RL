...
class FlockingEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        cohesion_reward, alignment_reward, collision_penalty, Collisions = self.calculate_reward()
        ...
        info["Reward"]=sum(info.values())
        reward=info["Reward"]

        # if (collision=="True"):    #Exit on collision
    ...
    def calculate_reward(self):
        ...
        Collisions = {}        #See if this is correct place
        ...
        for agent in self.agents:
            ...
            Collisions[agent] = collisions_agent
        ...
        return CohesionReward, AlignmentReward, CollisionPenalty, Collisions
...
Name = rf'Agents_{SimulationVariables["SimAgents"]}_PPO_{SimulationVariables["TrainTimeSteps"]}'

env = DummyVecEnv([lambda: FlockingEnv()])
model = PPO("MlpPolicy", env,  tensorboard_log="./ppo_Agents_tensorboard/")
# callback = HalveLearningRateCallback()
model.learn(total_timesteps=SimulationVariables["TrainTimeSteps"])  # Adjust the multiplier
...
for episode in range(1, RLVariables['Episodes']):
    ...
    with open(rf'agent_positionsTest_{episode}.json', 'w') as f:        #Add to params file
