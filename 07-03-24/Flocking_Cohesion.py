...
class FlockingEnv(gym.Env):
    ...
    def calculate_reward(self):
        ...
        for _, agent in enumerate(self.agents): 
            ...
            reward, out_of_flock=self.calculate_combined_reward(agent, neighbor_positions) #############################################################
    ...
    def calculate_combined_reward(self, agent, neighbor_positions):
        total_reward=0  
        out_of_flock=False
        ...
        if (len(neighbor_positions) > 0):
            for neighbor_position in neighbor_positions:
                ...
                #     reward_change = -100 * (1 - (distance / SimulationVariables["SafetyRadius"]))
                #     total_reward += reward_change
                elif (SimulationVariables["SafetyRadius"] < distance < SimulationVariables["NeighborhoodRadius"]):
                    # Decaying exponential function for rewards within the neighborhood radius
                    total_reward += 100
...
# Testing
env = FlockingEnv()
model = PPO.load(rf"{Files['Flocking']}\\Models\\FlockingWorking")
