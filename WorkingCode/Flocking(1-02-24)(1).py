...
class FlockingEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        reward = info["SeparationReward"] 
        
        if (info["Collisions"]==True and self.CTDE==True):
            print("Collision")
            done = True
            self.counter+=1
            # print(self.counter)
            
        with open(rf"{Files['Flocking']}\\Rewards\\Reward_Training_log.json", 'w') as f:
    ...
    def calculate_reward(self):
        ...
        with open(rf"{Files['Flocking']}\\Rewards\\Collision_log.json", 'w') as f:
            json.dump(CollisionPenalty, f, indent=4)

        with open(rf"{Files['Flocking']}\\Rewards\\Cohesion_log.json", 'w') as f:
            json.dump(CohesionReward, f, indent=4)

        with open(rf"{Files['Flocking']}\\Rewards\\Alignment_log.json", 'w') as f:
            json.dump(AlignmentReward, f, indent=4)

        with open(rf"{Files['Flocking']}\\Rewards\\Separation_log.json", 'w') as f:
            json.dump(SeparationReward, f, indent=4)
...
for episode in range(0, SimulationVariables['Episodes']):
    ...
    while((timestep <= SimulationVariables["EvalTimeSteps"]) and (not done)):
        ...
        print(reward)
        reward_log.append(round(reward,2))
    ...
    with open(rf"{Files['Flocking']}\\Rewards\\Episode_{episode}.json", 'w') as f:
            json.dump(reward_log, f, indent=4)
   
      
        
    
    with open(rf"{Files['Flocking']}\\Testing\\_{episode}.json", 'w') as f:        #Add to params file
