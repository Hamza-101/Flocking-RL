...
model.save(rf'Results\Flocking\{NAME}')
env.close()


env = FlockingEnv()
model = PPO.load(rf'Results\Flocking\{NAME}')
...
for episode in range(1, RLVariables['Episodes']):
    ...
    while((timestep <= SimulationVariables["EvalTimeSteps"]) and (not done)):
        ...
        with open(rf'{Results["EpRewards"]}{episode}.json', 'w') as f:
               json.dump(reward_log, f, indent=4)
        timestep = timestep + 1
      
        
    
    with open(rf'Files["Flocking"]' + {episode} + '.json', 'w') as f:        #Add to params file
