...
class EpisodeCounterCallback(BaseCallback):
    ...
    def _on_rollout_start(self) -> None:
        ...
        print(f"Episode Counter: {self.timestep_counter}")
...
for i in range(SimulationVariables["TotalEps"]):
    model.learn(total_timesteps=SimulationVariables["LearningTimeSteps"], callback=[ErrorHandler, episode_counter_callback])  # Adjust the multiplier
    env.envs[0].counter += 1
    print(f"FlockingEnv Episode Counter: {env.envs[0].counter}")
