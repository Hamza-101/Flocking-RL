...
class FlockingEnv(gym.Env):
    ...
    def calculate_reward(self):
        ...
        with open(rf"{Files['Flocking']}\\Separation_log.json", 'w') as f:
...
for episode in range(1, RLVariables['Episodes']):
    ...
    while((timestep <= SimulationVariables["EvalTimeSteps"]) and (not done)):
        ...
        with open(rf"{Files['Flocking']}\\Rewards\\Testing_{episode}.json", 'w') as f:
               json.dump(reward_log, f, indent=4)
        timestep = timestep + 1
      
        
    
    with open(rf"{Files['Flocking']}\\_{episode}.json", 'w') as f:        #Add to params file
