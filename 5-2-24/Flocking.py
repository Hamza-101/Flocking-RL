...
def delete_files(): 
    ...
    for episode in range(0, 9):  # You don't need to specify 0 as the starting point if you're iterating from 0
        for log_file in Logs:
            file_path = os.path.join(Files['Flocking'], f"Testing\\Rewards\\Components\\Episode{episode}", log_file)
            if os.path.exists(file_path):
                os.remove(file_path)
                print(f"File {file_path} has been deleted.")         
