...
class FlockingEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        # if(self.current_timestep % 600000 == 0):
    ...
    def calculate_reward(self):
        ...
        #             CohesionReward -= 10
        #         else:
        #             a = -20 / ((SimulationVariables['SafetyRadius'] - midpoint) ** 2)
        #             total_reward += a * (distance - midpoint) ** 2 + 10
    

    # if distance <= SimulationVariables["SafetyRadius"]:
                #     CohesionReward = 0
                # elif SimulationVariables["SafetyRadius"] < distance <= midpoint:
                #     CohesionReward += (20 / (midpoint - SimulationVariables["SafetyRadius"])) * (distance - SimulationVariables["SafetyRadius"])
                # elif midpoint < distance <= SimulationVariables["NeighborhoodRadius"]:
                #     CohesionReward += (-20 / (SimulationVariables["NeighborhoodRadius"] - midpoint)) * (distance - midpoint) + 10
    ...
    def reward(self, agent, neighbor_velocities, neighbor_positions):
        ...
        if len(neighbor_velocities) > 0:
            ...
            alignment = 1 - (orientation_diff / np.pi)
...
def generateVelocity():
    ...
    for episode in range(SimulationVariables["Episodes"]):
        ...
        for agent_id in range(len(env.agents)):
            ...
            # smoothed_velocities = savgol_filter(agent_velocities, window_length=5, polyorder=3, axis=0)
...
def generateAcceleration():
    for episode in range(SimulationVariables["Episodes"]):
        ...
        for agent_id in range(len(env.agents)):
            ...
            # accelerations_magnitude = np.sqrt(smoothed_accelerations[:, 0]**2 + smoothed_accelerations[:, 1]**2)
...
# distances = np.linspace(0, SimulationVariables["NeighborhoodRadius"], 500)
...
#     elif SimulationVariables["SafetyRadius"] < distance < midpoint:
#         ratio = (distance - SimulationVariables["SafetyRadius"]) / (midpoint - SimulationVariables["SafetyRadius"])
#         rewards[i] = 10 * np.exp(ratio - 1)
#     elif midpoint <= distance < SimulationVariables["NeighborhoodRadius"]:
#         ratio = (distance - midpoint) / (SimulationVariables["NeighborhoodRadius"] - midpoint)
#         rewards[i] = 10 * np.exp(-ratio)

# # Plot the graph
# plt.figure(figsize=(10, 6))
# plt.plot(distances, rewards, label="Cohesion Reward")
# plt.axvline(x=SimulationVariables["SafetyRadius"], color='r', linestyle='--', label="Safety Radius")
# plt.axvline(x=midpoint, color='g', linestyle='--', label="Midpoint")
# plt.axvline(x=SimulationVariables["NeighborhoodRadius"], color='b', linestyle='--', label="Neighborhood Radius")
# plt.xlabel("Distance")
# plt.ylabel("Cohesion Reward")
# plt.title("Cohesion Reward as a Function of Distance")
# plt.legend()
# plt.grid(True)
# plt.show()
