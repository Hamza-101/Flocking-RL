...
class FlockingEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        cohesion_reward, Collisions = self.calculate_reward()
    ...
    def calculate_reward(self):
        ...
        return CohesionReward, Collisions
...
Name = rf'Agents_Cohesion{SimulationVariables["SimAgents"]}_PPO_{SimulationVariables["LearningTimeSteps"]}'
...
for episode in range(1, RLVariables['Episodes']):
    ...
    while((timestep <= SimulationVariables["EvalTimeSteps"]) and (not done)):
        ...
        with open(rf'{Results["EpRewards"]}_Cohesion_{episode}.json', 'w') as f:      #See if we can improve
                json.dump(reward_log, f, indent=4)

        timestep = timestep + 1
      
    
    with open(rf'agent_positionsTestCohesion{episode}.json', 'w') as f:        #Add to params file
