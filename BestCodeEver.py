...
class FlockingEnv(gym.Env):
    ...
    def reward(self, agent, neighbor_velocities, neighbor_positions):
        ...
        if len(neighbor_positions) > 0:
            ...
            dot_product = np.dot(average_velocity, agent.velocity)      # U
            norm_product = np.linalg.norm(average_velocity) * np.linalg.norm(agent.velocity)        # V
...
def generateVelocity():
    ...
    for episode in range(SimulationVariables["Episodes"]):
        ...
        for agent_id in range(len(env.agents)):
            ...
            velocities_magnitude = np.sqrt(smoothed_velocities[:, 0]**2 + agent_velocities[:, 1]**2)  
...
def generateAcceleration():
    for episode in range(SimulationVariables["Episodes"]):
        ...
        for agent_id in range(len(env.agents)):
            ...
            accelerations_magnitude = np.clip(accelerations_magnitude, 0, 5) 

            # print()
            # print(agent_id)
            # print(accelerations_magnitude)
            # print()
